...
const reticle=new THREE.Mesh(new THREE.CircleGeometry(.0035,16),new THREE.MeshBasicMaterial({color:0xffffff,depthTest:false,depthWrite:false,transparent:false}));reticle.position.set(0,0,-.8);reticle.renderOrder=1001;camera.add(reticle);reticle.visible=false;
...
const drawer=drawerAction();drawer.userData.action=expandControls;drawer.userData.tapOnly=true;
const bringPrompt=floatingAction('◎  Bring video here','Aim here and press the headset button',2.15);bringPrompt.material.opacity=.68;bringPrompt.userData.action=bringVideoHere;bringPrompt.userData.tapOnly=true;
