...
function render(){const now=performance.now();if(active&&video.duration)updateTimeline(video.currentTime/video.duration);if(cardboard){
    ...
    if(controlsShown&&!hovered&&camera.quaternion.angleTo(ui.quaternion)>=THREE.MathUtils.degToRad(1)){ui.position.set(0,-1.05,-2.7).applyQuaternion(camera.quaternion).add(camera.position);ui.quaternion.copy(camera.quaternion)}
    if(menuCollapsed&&!hovered&&camera.quaternion.angleTo(drawer.quaternion)>=THREE.MathUtils.degToRad(1))placeInView(drawer,-.9,2.1);
