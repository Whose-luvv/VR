...
function render(){const now=performance.now();if(active&&video.duration)updateTimeline(video.currentTime/video.duration);if(cardboard){
    ...
    // A head-locked panel cannot be selected with a head-locked reticle. Keep a
    // wide capture zone so the dot can reach even the corner controls; only a
    // deliberate large turn brings the menu back into the current view.
